...
Engine.prototype.sync = function( cb ) {
  ...
  var self = this;
  self.soundcloud.get('users/' + self.config.soundcloud.slug + '/tracks', {
    limit: 100
  }, function(err, tracks) {
    if (err) {
      console.log(err);
    ...
    Show.query({}, function(err, recordings) {
      async.mapSeries( tracks , function(track, done) {
        var episode = _.find( recordings , function(e) {
          if (e.soundcloud == track.id) return true;
        ...
        form.append( 'soundcloud' , track.id );
        form.append( 'type', 'soundcloud' );
